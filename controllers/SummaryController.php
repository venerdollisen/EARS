...
class SummaryController extends Controller {
    ...
    public function getPendingApprovals() {
        ...
        try {
            [$dateFrom, $dateTo] = $this->resolveDateRange();
            $sql = "SELECT id, transaction_type, reference_no, payee_name, amount, transaction_date, cv_status, check_payment_status
                    FROM transactions
                    WHERE parent_transaction_id IS NULL
                      AND transaction_type IN ('cash_disbursement','check_disbursement')
                      AND (cv_status = 'Pending' OR check_payment_status = 'Pending')
    ...
    private function getTransactionCount($type) {
        $sql = "SELECT COUNT(*) as count FROM transactions WHERE transaction_type = ? AND parent_transaction_id IS NULL";
        $stmt = $this->db->prepare($sql);
        $stmt->execute([$type]);
    ...
    private function getTransactionCountByMonth($type) {
        $sql = "SELECT COUNT(*) as count FROM transactions WHERE transaction_type = ? AND parent_transaction_id IS NULL AND MONTH(transaction_date) = MONTH(CURRENT_DATE()) AND YEAR(transaction_date) = YEAR(CURRENT_DATE())";
        $stmt = $this->db->prepare($sql);
        $stmt->execute([$type]);
        $result = $stmt->fetch(PDO::FETCH_ASSOC);
        return $result['count'] ?? 0;
    ...
    private function getTransactionAmountByMonth($type) {
        $sql = "SELECT COALESCE(SUM(amount), 0) as total FROM transactions WHERE transaction_type = ? AND parent_transaction_id IS NULL AND MONTH(transaction_date) = MONTH(CURRENT_DATE()) AND YEAR(transaction_date) = YEAR(CURRENT_DATE())";
        $stmt = $this->db->prepare($sql);
        $stmt->execute([$type]);
        $result = $stmt->fetch(PDO::FETCH_ASSOC);
        return $result['total'] ?? 0;
    ...
    private function getAccountTypeSummary() {
        $sql = "SELECT 
                ...
                LEFT JOIN chart_of_accounts coa ON at.id = coa.account_type_id
    ...
    private function statusCounts(string $type, string $dateFrom, string $dateTo): array {
        $sql = "SELECT 
                    SUM(cv_status = 'Pending') AS pending,
                    SUM(cv_status = 'Approved') AS approved,
                    SUM(cv_status = 'Rejected') AS rejected
                FROM transactions
                WHERE parent_transaction_id IS NULL
    ...
    private function paymentFormSplit(string $dateFrom, string $dateTo): array {
        $sql = "SELECT payment_form, COUNT(*) AS cnt, COALESCE(SUM(amount),0) AS total
                FROM transactions
                WHERE parent_transaction_id IS NULL
                  AND transaction_date BETWEEN ? AND ?
